...
@export var camera_left_limit = -220
@export var camera_right_limit = 220
@export var camera_top_limit = -344
@export var camera_down_limit = 344
...
func _physics_process(delta):
	...
	#Limitamos la posicion de la camara para evitar que se salga de cuadro
	if position.x < camera_left_limit:
		position.x = camera_left_limit
	if position.x > camera_right_limit:
		position.x = camera_right_limit
	if position.y < camera_top_limit:
		position.y = camera_top_limit
	if position.y > camera_down_limit:
		position.y = camera_down_limit
